...
fn main() {
    let in_range = |line: Line| {
        let count = line.password.matches(line.digit).count() as i32;
        line.min <= count && count <= line.max
    };

    let in_position = |line: Line| {
        let passsword_chars = line.password.chars().collect::<Vec<char>>();
        let is_first = passsword_chars[(line.min - 1) as usize] == line.digit;
        let is_second = passsword_chars[(line.max - 1) as usize] == line.digit;
        is_first != is_second
    };
...
fn parse(row: &str) -> Line {
    lazy_static! {
        static ref RE: Regex =
            Regex::new(r"^(?P<min>\d+)-(?P<max>\d+) (?P<digit>[a-z]): (?P<password>[a-z]+)$")
                .unwrap();
    }
    match RE.captures(&row) {
        Some(cap) => Line {
            ...
            digit: cap.name("digit").unwrap().as_str().chars().next().unwrap(),
